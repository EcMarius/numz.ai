...
new class extends Component
{
    ...
    {
        // Rate limiting: 10 attempts per minute per IP (generous limit to avoid annoying users)
        $key = 'login-attempt:' . request()->ip();

        if (RateLimiter::tooManyAttempts($key, 10)) {
            $seconds = RateLimiter::availableIn($key);
            $this->addError('email', 'Too many login attempts. Please try again in ' . ceil($seconds / 60) . ' minute(s).');
            return;
        }
        ...
        if($this->twoFactorEnabled && !is_null($userAttemptingLogin->two_factor_confirmed_at)){
            ...
            // Clear rate limiter on successful 2FA initiation
        ...
        <x-auth::elements.container>
            ...
            <form wire:submit="authenticate" class="space-y-5">
                ...
                @if($showSocialProviderInfo)
                    ...
                    {{-- Social auth buttons are handled by the plugin system below --}}
                @endif
                ...
                <div class="flex items-center justify-between text-sm leading-5 {{ $showPasswordField ? '-mt-3' : '' }}">
            ...
            @if(app()->bound(\Wave\Plugins\PluginManager::class))
                ...
                @foreach($authPlugins as $plugin)
                    {!! $plugin->renderAuthButtons('login') !!}
                @endforeach
            @endif
