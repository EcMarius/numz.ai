        ...
        <div class="flex flex-col flex-wrap w-full mx-auto lg:max-w-4xl">
            ...
            <div class="h-full space-y-5">
                @foreach($plans as $plan)
                    ...
                    <div
                        {{--  Show plan if it has either price_id OR price for the selected billing cycle --}}
                        x-show="(billing_cycle_selected == 'month' && ('{{ $plan->monthly_price_id }}' != '' || '{{ $plan->monthly_price }}' != '')) || (billing_cycle_selected == 'year' && ('{{ $plan->yearly_price_id }}' != '' || '{{ $plan->yearly_price }}' != ''))"
                        x-init="
                        ...
                        <div class="flex flex-col mb-10 h-full bg-white dark:bg-neutral-800 rounded-xl ease-out duration-300 border-2 @if(request()->get('plan') == $plan->id) border-{{ config('devdojo.billing.style.color') }}-500 @else border-gray-200 dark:border-neutral-700 @endif shadow-sm sm:mb-0 group-hover:border-{{ config('devdojo.billing.style.color') }}-500">
